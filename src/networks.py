...
class RFFNet_mini(BaseNetwork):
    def __init__(self, in_channels=3, number_b=1, init_weights=True):
        self.inplanes = in_channels
        super(RFFNet_mini, self).__init__()

        # stem net

        # get 32 dimention feature
        self.layer1 = self._make_layer(3, 3, stride=1)
...
class RFFNet_medium(BaseNetwork):
    def __init__(self, in_channels=3, number_b=1, init_weights=True):
        self.inplanes = in_channels
        super(RFFNet_medium, self).__init__()
        ...
        self.stage4, pre_stage_channels = self._make_stage(
            num_blocks, pre_stage_channels, out_channels, stage_index=4)
        up_factor = 2 << 3
...
class TwoBranchModule(nn.Module):
    ...
    def _make_d_reduce(self, branch_index, out_channels,
        ...
        if self.stage > 0:
            if branch_index == 0:
                # in_c = self.num_inchannels[0] + self.out_channels[0]  older version
                in_c = self.num_inchannels[0] + (self.out_channels[1] >> (self.stage + 1))
            else:
